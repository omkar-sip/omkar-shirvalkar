...
const Sidebar = ({ state, onTake, onSave, onSelect }: SidebarProps) => {
  ...
  return (
    <div className={`${isMobile ? "relative w-full" : "absolute w-74 h-full z-1 left-0 top-0"} flex flex-col bg-zinc-900/85 backdrop-blur-xl`}>
      ...
      <div className={`text-xs ${isMobile ? "max-h-40" : "flex-1"} overflow-y-scroll`} p="t-5 b-2.5 x-2.5">
...
const FaceTime = () => {
  ...
  return (
    <div className={`${isMobile ? "flex flex-col" : "relative"} h-full`}>
      <Sidebar
        state={state}
        onTake={() => {
          if (!state.curImage) {
      ...
      <div className={`${isMobile ? "flex-1" : "h-full"} bg-zinc-800`}>
        {!state.curImage ? (
          <Webcam
            mirrored={true}
            audio={false}
