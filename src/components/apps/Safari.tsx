...
const NavSection = ({ width, section, setGoURL }: NavSectionProps) => {
  ...
  return (
    <div className="mx-auto w-full max-w-screen-md" p="t-8 x-4">
      ...
      {isMobile ? (
        // Mobile: Grid layout (Enforced Inline)
        <div 
          className="mt-4"
          style={{ 
            display: "grid", 
        ...
        >
          {section.sites.map((site: SiteData) => (
            ...
            >
              <div 
                className="rounded-md overflow-hidden bg-white mb-1 flex-shrink-0"
              ...
              >
                {site.img ? (
                  ...
                  <div
                    className="w-full h-full flex items-center justify-center cursor-pointer text-black p-0.5"
                    onClick={
                      site.inner ? () => setGoURL(site.link) : () => window.open(site.link)
                    }
                  >
                    <span className="text-[10px] leading-none text-center">{site.title}</span>
                  </div>
                )}
              </div>
              <span 
        ...
        // Desktop: Grid layout
        <div className="mt-3 grid grid-flow-row grid-cols-9">
          {section.sites.map((site: SiteData) => (
            <div key={`safari-nav-${site.id}`} className="h-28 flex flex-col items-center">
              <div className="size-16 rounded-md overflow-hidden bg-white">
                {site.img ? (
                  <img
                    src={site.img}
                    alt={site.title}
                  ...
                  <div
                    className="size-full flex-center cursor-pointer text-black"
                    onClick={
                      site.inner ? () => setGoURL(site.link) : () => window.open(site.link)
                    }
                  >
              ...
              <span className="mt-2 w-full text-center text-sm leading-tight">
                {site.title}
              </span>
            </div>
          ))}
