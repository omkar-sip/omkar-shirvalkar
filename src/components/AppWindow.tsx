...
const TrafficLights = ({ id, close, aspectRatio, max, setMax, setMin }: TrafficProps) => {
  ...
  const closeWindow = (e: React.MouseEvent | React.TouchEvent): void => {
    e.stopPropagation();
    playSound("/music/close-app.mp3");
    close(id);
  };

  const minimizeWindow = () => {
    playSound("/music/minimize-app.mp3");
    setMin(id);
  };

  const maximizeWindow = () => {
    playSound("/music/enlarge-window.mp3");
    setMax(id);
  };
