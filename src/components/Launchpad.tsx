        ...
        >
          {search().map((app) => (
            <div key={`launchpad-${app.id}`} h="32 sm:36" flex="~ col">
              <a
                className="w-14 sm:w-20 mx-auto flex-center"
                href={app.link}
                target="_blank"
                rel="noreferrer"
                onClick={(e) => e.stopPropagation()}
              >
                {app.img ? (
                  ...
                  <div className="size-14 sm:size-16 rounded-2xl bg-gradient-to-br from-white/20 to-white/5 border border-white/20 shadow-lg flex-center">
                    <span
                      className={`${app.icon} text-4xl sm:text-5xl text-white`}
                      title={app.title}
                    />
                  </div>
                )}
